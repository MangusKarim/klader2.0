...
function ensureJsonDb() {
  ...
  if (!fs.existsSync(dbFile)) {
    ...
    const initialData = {
      ...
      categories: ['Punjabi', 'Saree', 'Kurti', 'Polo', 'Sherwani', 'Pant', 'T-Shirt', 'Shoe'],
      settings: {
        companyName: 'Klader',
        currency: 'BDT',
  ...
  try {
    ...
    if (adminCheck.rows.length === 0) {
      ...
      await client.query(`
        ...
        ('Punjabi'), ('Saree'), ('Kurti'), ('Polo'), ('Sherwani'), ('Pant'), ('T-Shirt'), ('Shoe')
      `);
    }
...
export async function getDbData() {
  if (isPostgres) {
    try {
      ...
      const categoriesResult = await pool.query('SELECT name FROM categories ORDER BY name ASC');
